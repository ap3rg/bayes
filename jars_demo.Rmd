...
kidney_beans =  ### YOUR SAMPLE HERE
black_beans =  ## YOUR SAMPLE
...
1. Does the sample have more black beans than we need (i.e $> 0.7$)?
	
2. What is our "confidence" on this estimate?
...
Given data \( X \sim \mathrm{Binomial}(n, p_0) \) with \( n = \text{sample_size} \) and observed \( x_{\text{obs}} = \text{black_beans} \).
...
result <- binom.test(black_beans, sample_size, p = h0, alternative = "greater")
result$p.value

```
#### 2.  What is our "confidence" on this estimate?
Calculate the the 95% confidence interval for p>0.7.
...
From the Frequentist approach we have: 
...
However we don't know what our prior $P(H)$ is, instead we define it as a dristirbution that captures our prior believes. We will use the Beta distribution, since it provides both flexibility and mathematical tractability. 
...
```{r}
#  ADJUST YOUR PRIOR BASED ON THE INFORMATION UNDER THE JAR-LID
alpha_prior = ## YOUR PRIOR HERE
beta_prior = ## YOUR PRIOR HERE
...
y <- black_beans
p0 <- h0  
# Plot your beta to make sure it is sensible
# Create a grid of p values and corresponding Beta density
df <- data.frame(
  p = seq(0, 1, length.out = 1000)
)
df$density <- dbeta(df$p, alpha_prior, beta_prior)
...
# the multiplication analytically (as Elvira showed on the board).
...
# cred_int_lower = qbeta(0.025, alpha_post, beta_post)
# cred_int_upper = qbeta(0.975, alpha_post, beta_post)
```
